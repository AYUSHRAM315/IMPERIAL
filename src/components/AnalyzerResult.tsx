...
function ProsCons({ pros, cons, t }: { pros: string[]; cons: string[]; t: Translate }) {
...
function CoupleCard({ cr, index, t }: { cr: CoupleReading; index: number; t: Translate }) {
  ...
  return (
    <div className={`lux-card p-4 ${tierClass[tier]} animate-fade-up`} style={{ animationDelay: `${index * 60}ms` }}>
      ...
      {cr.data ? (
        <>
          <h4 className="mt-2 font-display text-lg text-cream-50">{t(cr.data.title)}</h4>
          <ProsCons pros={cr.data.pros} cons={cr.data.cons} t={t} />
        </>
      ) : (
        <p className="mt-2 text-sm text-cream-200/50 italic">{t('noSpecificPairReading')}</p>
      )}
    </div>
  );
}
...
export default function AnalyzerResult({ result, mobile, dob }: { result: AnalysisResult; mobile: string; dob: string | null }) {
  ...
  const saveLocal = async () => {
    ...
    try {
      ...
      setStoredJson(STORAGE_KEY, [row, ...existing]);
      setSaved(true);
    ...
    <div className="space-y-6">
      ...
      <div className="lux-card p-6 animate-fade-up">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-gold-400/70">{t('mobileTotalLabel')}</p>
            <div className="flex items-center gap-3 mt-1">
              <span className="font-display text-5xl font-semibold text-cream-50">{result.mobileTotal}</span>
              {result.gateway && (
                <span className={`text-xs px-3 py-1 rounded-full border ${tierBadgeClasses[result.gateway.tier]}`}>
                  {t(tierLabelKey[result.gateway.tier])} {t('gatewayLabel')}
                </span>
              )}
            </div>
          ...
          <div className="flex gap-2">
            <button onClick={saveLocal} disabled={saving} className="lux-btn">
              {saving ? t('saving') : (saved ? t('saved') : t('saveReading'))}
            </button>
          </div>
        </div>
        ...
        <div className={`mt-4 p-4 rounded-xl bg-espresso-950/40 ${tierClass[result.gateway.tier]}`}>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-sm font-medium text-cream-50">{t('masterGatewayPair', { couple: result.gateway.couple })}</span>
          </div>
          <p className="text-sm text-cream-200/80">{t('gatewayInsight')}</p>
        </div>
      )}
      </div>
      ...
      {result.rootInfo && (
        <div className={`lux-card p-6 animate-fade-up ${tierClass[result.rootInfo.tier]}`}>
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="font-display text-2xl text-cream-50">{t('rootMatch')}</h3>
          </div>
          <div className="grid sm:grid-cols-3 gap-4 text-sm">
            <div>
              <p className="text-cream-200/60">{t('rootNumberLabel')}</p>
              <p className="font-display text-3xl text-gold-300">{result.rootInfo.rootNum}</p>
            </div>
            <div>
              <p className="text-cream-200/60">{t('rulingPlanetLabel')}</p>
              <p className="text-cream-50 font-medium">{result.rootInfo.planet}</p>
              <p className="text-cream-200/60 text-xs">{t('personaLabel', { persona: result.rootInfo.persona })}</p>
            </div>
            <div>
              <p className="text-cream-200/60">{t('mobileTotalLabel')}</p>
              <p className="font-display text-3xl text-cream-50">{result.mobileTotal}</p>
              <span className={`text-xs px-2 py-0.5 rounded-full border ${tierBadgeClasses[result.rootInfo.tier] || tierBadgeClasses.neutral}`}>
                {t(tierLabelKey[result.rootInfo.tier] || 'neutral')}
              </span>
            </div>
          </div>
          <p className="mt-4 text-sm text-cream-100/90">{t('rootInsight')}</p>
          <div className="mt-3 flex flex-wrap gap-2 text-xs">
            <span className="px-2 py-1 rounded-full bg-emerald-400/10 text-emerald-300 border border-emerald-400/20">{t('luckyLabel', { values: result.rootInfo.lucky.join(', ') })}</span>
            <span className="px-2 py-1 rounded-full bg-gold-400/10 text-gold-300 border border-gold-400/20">{t('neutralLabel', { values: result.rootInfo.neutral.join(', ') })}</span>
            <span className="px-2 py-1 rounded-full bg-rose-400/10 text-rose-300 border border-rose-400/20">{t('cautionLabel', { values: result.rootInfo.bad.join(', ') })}</span>
          </div>
        </div>
      )}
      ...
      {result.runs.length > 0 && (
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-display text-2xl text-cream-50">{t('repeatedDigitAlertsLabel')}</h3>
          </div>
          {result.runs.map((r, i) => (
            <div key={i} className="lux-card p-5 tier-bad animate-fade-up" style={{ animationDelay: `${i * 80}ms` }}>
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs px-2 py-0.5 rounded-full bg-rose-400/15 text-rose-300 border border-rose-400/30">
                  {t('repeatCount', { count: r.length })}
                </span>
              </div>
              <h4 className="mt-2 font-display text-lg text-cream-50">{t(r.data.title)}</h4>
              <ProsCons pros={r.data.pros} cons={r.data.cons} t={t} />
            </div>
          ))}
        </div>
      )}
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-display text-2xl text-cream-50">{t('pairReadingsLabel')}</h3>
          <span className="text-xs text-cream-200/50">{t('masterGatewayFirst')}</span>
        </div>
        {reversedCouples.map((cr, i) => (
          <CoupleCard key={i} cr={cr} index={i} t={t} />
        ))}
      </div>
